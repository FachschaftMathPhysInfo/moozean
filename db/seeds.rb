...
for i in 0..50
  ...
  report= Report.create(pdf:'test'+rand(0..300).to_s,tex:'tex'+rand(0..300).to_s,examination_date:Faker::Date.between(2.days.ago, Date.tomorrow),subject:Subject.offset(rand(Subject.count)).first,typ:Typ.offset(rand(Typ.count)).first)
