...
for i in 0..50
  ...
  report= Report.create(pdf:File.read('erd.pdf'),tex:'tex'+rand(0..300).to_s,examination_at:Faker::Date.between(2.days.ago, Date.tomorrow),subject:Subject.offset(rand(Subject.count)).first,typ:Typ.offset(rand(Typ.count)).first,picture:Faker::LoremPixel.image("150x150",true))
